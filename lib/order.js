Array.prototype.order = function (expression) {
    ...
    if (expression) {
        ...
        if (typeOf === 'string')
            return this.sort((item1, item2) => expression(item1).localeCompare(condition(item2)));
...
Array.prototype.orderDesc = function (expression) {
    ...
    if (expression) {
        ...
        let typeOf = typeof (expression(this[0]));
        if (typeOf === 'string')
            return this.sort((item1, item2) => expression(item2).localeCompare(condition(item1)));
